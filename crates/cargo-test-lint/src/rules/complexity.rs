...
impl Rule for DeepWrapper {
    ...
    fn validate(&self, ctx: &RuleContext, query_match: &QueryMatch) -> Vec<Diagnostic> {
        ...
        let (Some(ty_node), Some(item_node)) = (ty_capture.map(|c| c.node), item_capture.map(|c| c.node)) else { return vec![]; };
        let depth = count_generic_depth(&ty_node, 0);
        if depth > 3 {
            vec![Diagnostic {
                rule_id: self.id().into(),
                level: self.default_level(),
                message: format!("deeply nested type wrapper ({depth} levels) — test setup is overly complex"),
...
fn count_generic_depth(node: &tree_sitter::Node, current: usize) -> usize {
    let kind = node.kind();
    let is_wrapper = matches!(kind, "generic_type" | "tuple_type" | "reference_type" | "array_type");
