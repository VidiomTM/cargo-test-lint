...
mod tests {
    ...
    fn unbound_file_create_flagged() {
        ...
        assert_eq!(diags.len(), 1, "unbound File::create should produce exactly 1 diagnostic");
        assert!(diags[0].message.contains("File::create"), "message should reference File::create");
    }
    ...
    fn bound_file_create_passes() {
        ...
        assert_eq!(
            ...
            0,
            "let-bound File::create should produce no diagnostics"
        );
    }
    ...
    fn bound_with_unwrap_passes() {
        ...
        assert_eq!(
            test_rule(&MissingDropGuard, source).len(),
            0,
            "let-bound with unwrap should produce no diagnostics"
        );
    }
    ...
    fn bound_with_question_mark_passes() {
        ...
        assert_eq!(
            test_rule(&MissingDropGuard, source).len(),
            0,
            "let-bound with ? should produce no diagnostics"
        );
    }
    ...
    fn bound_with_method_chain_passes() {
        ...
        assert_eq!(
            test_rule(&MissingDropGuard, source).len(),
            0,
            "let-bound method chain should produce no diagnostics"
        );
    }
    ...
    fn unbound_tempdir_flagged() {
        ...
        let diags = test_rule(&MissingDropGuard, source);
        assert_eq!(diags.len(), 1, "unbound TempDir::new should produce exactly 1 diagnostic");
    }
    ...
    fn tempfile_tempdir_recognized() {
        ...
        assert_eq!(
            test_rule(&MissingDropGuard, source).len(),
            0,
            "let-bound tempfile::tempdir should produce no diagnostics"
        );
    }
    ...
    fn unbound_tempfile_tempdir_flagged() {
        ...
        let diags = test_rule(&MissingDropGuard, source);
        assert_eq!(diags.len(), 1, "unbound tempfile::tempdir should produce exactly 1 diagnostic");
    }
    ...
    fn tcp_listener_without_binding_flagged() {
        ...
        assert_eq!(
            test_rule(&MissingDropGuard, source).len(),
            1,
            "unbound TcpListener::bind should produce exactly 1 diagnostic"
        );
    }
    ...
    fn wildcard_let_flagged() {
        ...
        let diags = test_rule(&MissingDropGuard, source);
        assert_eq!(diags.len(), 1, "wildcard let should produce exactly 1 diagnostic");
    }
    ...
    fn passed_as_argument_not_let_bound_still_flagged() {
        ...
        assert_eq!(
            test_rule(&MissingDropGuard, source).len(),
            1,
            "resource passed as argument without let binding should be flagged"
        );
    }
    ...
    fn let_bound_then_passed_ok() {
        ...
        assert_eq!(
            test_rule(&MissingDropGuard, source).len(),
            0,
            "let-bound resource then passed should produce no diagnostics"
        );
    }
}
