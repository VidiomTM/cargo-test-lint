...
"#;
        ...
        assert_eq!(diags.len(), 1, "if in test should produce exactly one diagnostic");
        assert_eq!(
            diags[0].rule_id, "CTL_TEST_BRANCHING",
            "diagnostic rule_id should be CTL_TEST_BRANCHING"
        );
    }
...
"#;
        let diags = test_rule(&rule(), source);
        assert_eq!(diags.len(), 1, "match in test should produce exactly one diagnostic");
    }
...
"#;
        let diags = test_rule(&rule(), source);
        assert_eq!(diags.len(), 0, "non-test function should be ignored");
    }
}
