...
"#;
        ...
        assert_eq!(diags.len(), 1, "bare assert should produce exactly 1 diagnostic");
        assert_eq!(diags[0].rule_id, "CTL_ASSERT_MSG", "diagnostic should be CTL_ASSERT_MSG");
    }
...
"#;
        let diags = test_rule(&rule(), source);
        assert_eq!(diags.len(), 1, "bare assert_eq should produce exactly 1 diagnostic");
    }
...
"#;
        let diags = test_rule(&rule(), source);
        assert_eq!(diags.len(), 1, "bare assert_ne should produce exactly 1 diagnostic");
    }
...
"#;
        let config = config_with_max(5);
        let diags = test_rule_with_config(&MaxExpects, source, config);
        assert_eq!(diags.len(), 1, "over threshold should produce exactly 1 diagnostic");
        assert!(
            diags[0].message.contains("6 assertions"),
            "message should mention assertion count"
        );
        assert!(diags[0].message.contains("max 5"), "message should mention threshold");
    }
...
"#;
        ...
        let diags = test_rule_with_config(&MaxExpects, source, config);
        assert_eq!(diags.len(), 0, "zero threshold should disable check entirely");
    }
}
